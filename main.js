...
(function () {
  ...
  const cityTypePoints = {
    ...
    housing: 0, education: 0, transport: 0, industryHeavy: 0, welfare: 0, governance: 0
  };
  ...
  // 重み
  const W = { alpha: 0.5, beta: 0.35, gamma: 0.15, delta: 0.12, zeta: 0.2, hysteresisType: 0.07, hysteresisLv: 0.05 };
  ...
  function selectChoice(choice) {
    ...
    if (choice.effects) {
      for (const k in choice.effects)
        if (status[k] !== undefined) status[k] = clamp(status[k] + choice.effects[k], 0, 100);
    }

    if (choice.typePoints) {
      for (const k in choice.typePoints) {
        const key = (TYPE_ALIAS[k] || k).toLowerCase();
        if (cityTypePoints[key] !== undefined) cityTypePoints[key] += choice.typePoints[k];
      }
    }
  ...
  function makeFeedback(status, resources) {
    ...
    if (resources.tech > 10) good.push("技術活用が進んでいます");
    if (resources.recycled > 5) good.push("循環型社会に近づいています");

    if (status.env < 30) bad.push("環境が悪化しています");
    if (status.eco < 30) bad.push("経済が停滞しています");
    if (status.soc < 30) bad.push("社会的なつながりが弱まっています");
    if (resources.funds < 10) bad.push("資金が不足しています");
    if (resources.energy < 5) bad.push("エネルギーが足りません");
    if (resources.food < 5) bad.push("食料自給が課題です");
  ...
  function updateEvidence(result) {
    ...
    evidenceEl.innerHTML = `
      ...
      <div style="margin-top:6px">
        類似度:${best.parts.cos.toFixed(2)}　資源整合:${best.parts.resAff.toFixed(2)}　
        シナジー:${best.parts.syn.toFixed(2)}　リスク:${best.parts.risk.toFixed(2)}
      </div>
  ...
  function endGame() {
    ...
    summary.innerHTML = `
      <h3>都市の振り返り</h3>
      <p>${fb}</p>
      <p>あなたの選択は、この都市を一つの理想形へ導きました。<br>
      しかし、完璧な都市は存在しません。次はどんな未来を目指しますか？</p>
    `;
  ...
  function runAutoSim100() {
    ...
    for (let i = 0; i < N; i++) {
      ...
      for (let r = 0; r < 20; r++) {
        ...
        if (ch.effects) for (const k in ch.effects) if (st[k] !== undefined) st[k] = clamp(st[k] + ch.effects[k], 0, 100);
        if (ch.resources) for (const k in ch.resources) rs[k] = Math.max(0, (rs[k] || 0) + ch.resources[k]);
        if (ch.typePoints) for (const k in ch.typePoints) tp[k] = (tp[k] || 0) + ch.typePoints[k];
      }
